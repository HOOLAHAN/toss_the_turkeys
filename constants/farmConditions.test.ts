...
describe('farm conditions',()=>{
  it('rotates after every complete player round',()=>{expect(farmConditionForTurn(0,3).id).toBe('clear');expect(farmConditionForTurn(2,3).id).toBe('clear');expect(farmConditionForTurn(3,3).id).toBe('windy');expect(farmConditionForTurn(18,3).id).toBe('clear')});
