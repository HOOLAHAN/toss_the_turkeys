...
export function applyFarmCondition(result:TossScoreResult,condition:FarmCondition){if(result.isPlucked||result.isFowlPlay)return result;if(condition.id==='harvest'){const bonus=[result.turkeyA,result.turkeyB].filter(p=>p==='jiblets'||p==='wattle_wobble').length*5;return bonus?{...result,points:result.points+bonus}:result;}if(condition.id==='golden'&&result.points>0)return {...result,points:result.points+2};if(condition.id==='fox'&&result.points>0)return {...result,points:result.points+5};return result;}
