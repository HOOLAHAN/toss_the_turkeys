...
export default function Play(){
  ...
  const rest=(a=arena)=>{const y=a.height-76-turkeySize,xs=[a.width*.34-turkeySize/2,a.width*.66-turkeySize/2];bodies.current=xs.map(x=>({x,y,vx:0,vy:0,angle:0,spin:0,grounded:true}));xs.forEach((x,i)=>{xy[i].setValue({x,y});angles[i].setValue(0);});};
  ...
  return <Screen scroll={false} style={s.screen}>
    ...
    <View style={[s.arena,compact&&s.arenaCompact]} onLayout={layout} {...pan.panHandlers}><View style={s.halo}/><View style={s.ground}><View style={s.grass}/></View><FlyingTurkey position={shown?.turkeyA??'gobbler'} size={turkeySize} xy={xy[0]} angle={angles[0]}/><FlyingTurkey position={shown?.turkeyB??'turkey_trot'} size={turkeySize} xy={xy[1]} angle={angles[1]}/><FeatherBurst burst={burst}/>
    ...
    <View style={s.actions}><GameButton style={s.compactButton} title={game.status==='animating'?'TURKEYS IN FLIGHT…':'QUICK TOSS'} onPress={()=>launch()} disabled={game.status==='animating'}/><GameButton style={s.compactButton} title={game.turnScore?`BANK ${game.turnScore} POINTS`:'BANK POINTS'} variant="secondary" onPress={bank} disabled={!game.turnScore||game.status==='animating'}/></View>
  </Screen>;
}

const s=StyleSheet.create({screen:{gap:6},headerRow:{flexDirection:'row',alignItems:'center',minHeight:44},heading:{flex:1,alignItems:'center'},close:{width:34,height:34,borderRadius:17,backgroundColor:'rgba(91,45,29,.08)',alignItems:'center',justifyContent:'center'},closeText:{fontFamily:'Nunito_400Regular',fontSize:27,lineHeight:29,color:C.brown},turnBadge:{width:34,height:34,borderRadius:17,backgroundColor:C.brown,alignItems:'center',justifyContent:'center'},turnBadgeText:{fontFamily:'Nunito_800ExtraBold',fontSize:10,color:C.gold},eyebrow:{fontFamily:'Nunito_800ExtraBold',fontSize:8,letterSpacing:1.8,color:C.muted},turn:{fontFamily:'Baloo2_800ExtraBold',fontSize:23,lineHeight:29,paddingTop:2,color:C.ink},scores:{gap:5,paddingVertical:0},scoreChip:{minWidth:72,paddingVertical:2,paddingHorizontal:9,borderRadius:11,backgroundColor:'rgba(255,255,255,.7)',borderWidth:1.5,borderColor:C.line,alignItems:'center'},activeChip:{backgroundColor:C.brown,borderColor:C.brown},chipName:{fontFamily:'Nunito_700Bold',fontSize:10,color:C.muted},chipScore:{fontFamily:'Baloo2_700Bold',fontSize:17,lineHeight:20,paddingTop:1,color:C.ink},activeText:{color:C.white},counters:{flexDirection:'row',minHeight:56,backgroundColor:'rgba(255,255,255,.88)',borderRadius:15,paddingVertical:2,borderWidth:1.5,borderColor:C.line},counterHalf:{flex:1,alignItems:'center',justifyContent:'center'},counterLabel:{fontFamily:'Nunito_800ExtraBold',fontSize:8,lineHeight:11,letterSpacing:1,color:C.muted},counter:{fontFamily:'Baloo2_800ExtraBold',fontSize:27,lineHeight:33,paddingTop:2,color:C.brown},risk:{color:C.red},divider:{height:34,width:1,backgroundColor:C.line,alignSelf:'center'},arena:{flex:1,minHeight:390,borderRadius:24,overflow:'hidden',backgroundColor:'rgba(255,250,238,.55)',borderWidth:1,borderColor:'rgba(230,205,168,.65)'},arenaCompact:{minHeight:335},halo:{position:'absolute',alignSelf:'center',top:28,width:240,height:240,borderRadius:120,backgroundColor:'rgba(247,204,108,.22)'},ground:{position:'absolute',left:0,right:0,bottom:68,height:8,backgroundColor:'#8C6437',borderTopWidth:1,borderTopColor:'#315F2C'},grass:{height:4,backgroundColor:'#5E8B3D'},physicsTurkey:{position:'absolute',left:0,top:0},prompt:{position:'absolute',alignSelf:'center',top:'19%',alignItems:'center'},arrow:{fontFamily:'Nunito_800ExtraBold',fontSize:42,lineHeight:45,color:C.orange},promptTitle:{fontFamily:'Baloo2_800ExtraBold',fontSize:18,color:C.brown},hint:{fontFamily:'Nunito_700Bold',fontSize:10,color:C.muted},aim:{position:'absolute',alignSelf:'center',bottom:75,width:4,borderRadius:4,backgroundColor:C.orange},resultCard:{position:'absolute',alignSelf:'center',bottom:5,minWidth:'72%',alignItems:'center',backgroundColor:C.white,borderRadius:14,paddingHorizontal:14,paddingVertical:3,borderWidth:1.5,borderColor:C.line},danger:{backgroundColor:C.red,borderColor:C.redDark},gold:{backgroundColor:'#FFF1B7',borderColor:C.orange},resultTitle:{fontFamily:'Baloo2_800ExtraBold',fontSize:18,lineHeight:24,paddingTop:2,color:C.brown},names:{fontFamily:'Nunito_700Bold',fontSize:9,color:C.muted},points:{fontFamily:'Baloo2_800ExtraBold',fontSize:21,lineHeight:27,paddingTop:1,color:C.green},light:{color:C.white},lightMuted:{color:'#FFE6D8'},flying:{fontFamily:'Baloo2_800ExtraBold',fontSize:17,lineHeight:23,paddingTop:2,color:C.red,letterSpacing:1.5},featherOrigin:{position:'absolute',left:'50%',top:'48%'},feather:{position:'absolute',fontSize:23,color:'#A75C35'},actions:{gap:5},compactButton:{minHeight:44,borderRadius:14}});
