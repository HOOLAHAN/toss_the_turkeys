...
export default function How() {
  return (
    <Screen>
      ...
      <View style={[s.plucked,s.fowlPlay]}>
        <View style={s.warningBadge}><Text style={s.fowlMark}>×2</Text></View>
        <View style={s.pluckedCopyWrap}><Text style={s.pluckedTitle}>TOUCHING TURKEYS = FOWL PLAY!</Text><Text style={s.pluckedCopy}>This rare landing ends the turn and loses its unbanked points. Banked points remain safe.</Text></View>
      </View>
      <View style={s.tip}><Text style={s.tipLabel}>COMPLETE SCORING</Text><Text style={s.note}>Different poses add their base values. Matching scoring poses earn 4× their base value. Matching zero-point poses score <Text style={s.noteStrong}>1 point</Text>. Double Wattle Wobble scores 60 and triggers <Text style={s.noteStrong}>Thanksgiving!</Text></Text><View style={s.ruleLine}/><Text style={s.conditionNote}>When Farm Conditions are enabled: Harvest Festival adds +5 for each Jiblets or Wattle Wobble; Golden Hour adds +2 to a scoring toss; Fox Alert adds +5 to a scoring toss. Plucked It never receives a bonus.</Text></View>
    </Screen>
  );
}
...
const s = StyleSheet.create({
  ...
  plucked: { flexDirection: 'row', alignItems: 'center', gap: 13, backgroundColor: C.red, borderRadius: 20, padding: 15, marginTop: 17, borderBottomWidth: 4, borderBottomColor: C.redDark }, fowlPlay:{backgroundColor:C.brown,borderBottomColor:'#3F2118',marginTop:10}, warningBadge: { width: 42, height: 42, borderRadius: 21, backgroundColor: C.white, alignItems: 'center', justifyContent: 'center' }, warningMark: { fontFamily: 'Baloo2_800ExtraBold', fontSize: 28, lineHeight: 38, color: C.red }, fowlMark:{fontFamily:'Baloo2_800ExtraBold',fontSize:17,color:C.brown}, pluckedCopyWrap: { flex: 1 }, pluckedTitle: { fontFamily: 'Baloo2_800ExtraBold', fontSize: 17, lineHeight: 24, paddingTop: 2, color: C.white }, pluckedCopy: { fontFamily: 'Nunito_700Bold', fontSize: 11, lineHeight: 16, color: '#FFE3D1' },
  tip: { backgroundColor: 'rgba(255,255,255,.5)', borderRadius: 16, padding: 14, marginTop: 12 }, tipLabel: { fontFamily: 'Nunito_800ExtraBold', fontSize: 9, letterSpacing: 1.5, color: C.orange }, note: { fontFamily: 'Nunito_700Bold', fontSize: 12, lineHeight: 18, color: C.brown, marginTop: 3 }, noteStrong: { color: C.red }, ruleLine:{height:1,backgroundColor:C.line,marginVertical:9},conditionNote:{fontFamily:'Nunito_700Bold',fontSize:10,lineHeight:15,color:C.muted},
});
